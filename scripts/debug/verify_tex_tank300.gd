...
func _initialize() -> void:
	root.get_node("GameState").selected_model_index = 3
...
func _process(_d: float) -> bool:
	...
	if _f == 5:
		...
		for w in _car._wheels:
			var pivot := w["pivot"] as Node3D
			print("  wheel front=", w["front"], " pos=", pivot.position)
		var model := _car.get_node("Model").get_child(0) as Node3D
		print("模型 scale: ", model.scale, " visible: ", model.visible)
	elif _f == 140:
		root.get_texture().get_image().save_png("/tmp/tex_tank300.png")
		return true
	return false
