...
func _initialize() -> void:
	var model: Node3D = load("res://assets/vehicles/camaro_parts_textured.glb").instantiate()
	...
	root.add_child(cam)
	cam.position = Vector3(4.5, 2.5, -5.5)
...
func _process(_d: float) -> bool:
	...
	if _f == 3:
		...
		cam.look_at_from_position(Vector3(4.5, 2.5, -5.5), Vector3(0, 0.5, 0), Vector3.UP)
		cam.make_current()
	elif _f == 30:
		root.get_texture().get_image().save_png("/tmp/camaro_static.png")
		return true
	return false
